### Usage: python vidOnly.py IP_ADDRESS:8000
...
def show_video(readColors):
    redUpper = np.array([255, 255, 255], dtype=np.uint8) # Upper threshold for chassis ID
    redLower = np.array([0, 100, 0], dtype=np.uint8) #Lower threshold for chassis ID
    ...
    kernel = np.ones((5,5), np.uint8) # Decides nature of changes in erosion and dialation
    ...
    boardImg = readColors # Don't touch boardImg because we use RGB detection there
...
### Orientation ###
# Orient robot based on board centroids position vs chassis centroid
...
URL =  "http://" + sys.argv[1] + "/stream.mjpg" # Blend URL for passing IP
stream = urllib.urlopen(URL) # Open URL

bytes=''
# Encoding loop to grab new frames
while True:
    ...
    if a!=-1 and b!=-1:
        ...
        # Kill program on ESC key
        if cv2.waitKey(1) ==27:
            exit(0)

# Set mouse callback for click function
cv2.namedWindow("Original")
cv2.setMouseCallback("Original", click) # Calls click() when original picture is clicked on
