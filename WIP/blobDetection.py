...
def show_video(jpg):
    redUpper = np.array([90, 160, 150], dtype=np.uint8) # Upper threshold for chassis ID # HSV VERSION
    redLower = np.array([0, 129, 100], dtype=np.uint8) #Lower threshold for chassis ID # HSV VERSION
    
    #redUpper = np.array([100, 50, 200], dtype=np.uint8) # Upper threshold for chassis ID # HSV VERSION
    #redLower = np.array([10, 10, 0], dtype=np.uint8) #Lower threshold for chassis ID # HSV VERSION
    ...
    cv2.imshow("example", maskChassis)
    cv2.imshow("example2", maskBoard)

    centroidChassis = blobChassis(maskChassis)
    centroidBoard = blobBoard(maskBoard)

    try:
        xC = int(centroidChassis[0].pt[0])
        yC = int(centroidChassis[0].pt[1])
        xB = int(centroidBoard[0].pt[0])
        yB = int(centroidBoard[0].pt[1])

        
        cv2.circle(origPic, (xC, yC), 10, (0, 0, 255), -1)
        cv2.circle(origPic, (xB, yB), 10, (255, 0, 0), -1)
    except:
        pass
...
def blobChassis(maskChassis):
    ...
    reversemask = 255-maskChassis
    keypoints = detector.detect(reversemask)
    im_with_keypoints = cv2.drawKeypoints(maskChassis, keypoints, np.array([]), (0, 0, 255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    cv2.imshow("blobChassis", im_with_keypoints)
    return keypoints

def blobBoard(maskBoard):
    params = cv2.SimpleBlobDetector_Params()
    params.minThreshold = 0
    params.maxThreshold = 250
    ...
    detector = cv2.SimpleBlobDetector(params)
    reversemask = 255-maskBoard
    keypoints = detector.detect(reversemask)
    im_with_keypoints = cv2.drawKeypoints(maskBoard, keypoints, np.array([]), (0, 0, 255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    cv2.imshow("blobBoard", im_with_keypoints)
    return keypoints
